<?
if (!$inpswd) {  // Form을 통해 값이 입력되었는지를 검사
  echo ("<SCRIPT>
               window.alert('비밀번호를 입력해 주세요.');    history.go(-1);
            </SCRIPT>");  exit;
}
...
$res = mysql_query( "select * from Join_Membership where num = $num", $conn);
$rec = mysql_fetch_array($res);
if ($inpswd == $rec[password] ) {
    $query = "delete from Join_Membership where num = $num";
	$query1 = "drop table `{$id}_membership_samu_DB`";
	...
	mysql_query ( $query1, $conn );	// 회원 samu DB를 삭제한다.
	mysql_query ( $query2, $conn );	// 회원 schedule DB를 삭제한다.
	mysql_query ( $query3, $conn );	// 회원 message DB를 삭제한다.
} else {
   echo ("<SCRIPT>
               window.alert('비밀번호를 확인해주세요.');    history.go(-1);
            </SCRIPT>");  exit;
}
mysql_close($conn);                 // MySQL 서버 접속 종료   
echo(" <script>location='Membership_list.php';</script>"); // 자동 이동
?>
